...
foreach($templates as $relativePath) {
	...
	$checks = array(
		...
		array(strpos($template, '$maparray = mapTilesForCoordinates($xfull, $yfull);') !== false,
			'consulta los datos específicos del oasis'),
		array(strpos($template, '$username = (string)$maparray[$index][\'map_username\'];') !== false,
			'usa al propietario real del oasis'),
		array(strpos($template, '$maparray[$index][\'map_oasis_village\']') !== false,
		...
		array(strpos($template, '$targetalliance = (int)$maparray[$index][\'map_alliance\'];') !== false,
			'resuelve la alianza desde el propietario seleccionado'),
		array(strpos($template, '$tribe = (int)$maparray[$index][\'map_tribe\'];') !== false,
