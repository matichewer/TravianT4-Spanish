...
heroBucketAssert(mysqli_query($database->connection,
	"CREATE TEMPORARY TABLE $heroTable (uid INT UNSIGNED NOT NULL, wref INT UNSIGNED NOT NULL, home INT UNSIGNED NOT NULL DEFAULT 0, dead TINYINT NOT NULL, health FLOAT NOT NULL, PRIMARY KEY(uid)) ENGINE=MyISAM"),
...
heroBucketAssert(mysqli_query($database->connection,
	"INSERT INTO $trainingTable (id,vref,unit) VALUES (1,910102,0)"),
	'Could not seed paid revival');

heroBucketAssert(mysqli_query($database->connection,"UPDATE $heroTable SET home=wref"),'Could not set hero homes');
$result = $database->consumeHeroRevivalBucket(910001,1,910102);
heroBucketAssert($result['ok'] && $result['vref']===910101,'Home village did not override an old revival queue');
...
heroBucketAssert(mysqli_query($database->connection,"UPDATE $heroTable SET wref=910102,dead=1,health=0 WHERE uid=910001"),'Could not reset hero');
...
heroBucketAssert($result['ok'] && $result['vref']===910101,'Ordinary bucket revival did not use home village');
$hero = mysqli_fetch_assoc(mysqli_query($database->connection,"SELECT wref,dead,health FROM $heroTable WHERE uid=910001"));
$placedResult = mysqli_query($database->connection,"SELECT SUM(u.hero) AS total,MAX(CASE WHEN u.hero=1 THEN u.vref ELSE 0 END) AS hero_vref FROM $unitsTable AS u INNER JOIN $villagesTable AS v ON v.wref=u.vref WHERE v.owner=910001");
heroBucketAssert($placedResult!==false,'Could not inspect ordinary hero placement: '.mysqli_error($database->connection));
$placed = mysqli_fetch_assoc($placedResult);
heroBucketAssert((int)$hero['wref']===910101 && (int)$hero['dead']===0 && (float)$hero['health']===100.0,'Ordinary bucket revival left incorrect hero state');
heroBucketAssert((int)$placed['total']===1 && (int)$placed['hero_vref']===910101,'Ordinary bucket revival did not leave exactly one hero');
...
heroBucketAssert(mysqli_query($database->connection,"UPDATE $heroTable SET home=910101 WHERE uid=910002"),'Could not seed invalid home');
$result = $database->consumeHeroRevivalBucket(910002,4,910201);
