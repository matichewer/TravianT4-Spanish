...
<table cellpadding="1" cellspacing="1" id="details" class="transparent">
    ...
    <?php 
            ...
            echo "<tr><th>Edad</th><td>$age</td></tr>";
            }
			//Gender
            if(isset($displayarray['gender']) && $displayarray['gender'] != 0) {
            $gender = ($displayarray['gender']== 1)? "Masculino" : "Femenino";
            echo "<tr><th>Género</th><td>".$gender."</td></tr>";
            }
			//Location
            if($displayarray['location'] != "") {
            echo "<tr><th>Ubicación:</th><td>".$displayarray['location']."</td></tr>";
            }
            ?>
    		<tr>
        	<?php
            	if($_GET['uid'] == $session->uid) {
                	if($session->is_sitter){
            			echo "<td colspan=\"2\"> <span class=\"a arrow disabled\">Editar perfil</span></td>";
                    }else{
                    	echo "<td colspan=\"2\"> <a class=\"arrow\" href=\"spieler.php?s=1\">Editar perfil</a></td>";
                    }
                } else {
            		echo "<td colspan=\"2\"> <a class=\"message messageStatus messageStatusUnread\" href=\"nachrichten.php?t=1&amp;id=".$_GET['uid']."\">Enviar mensaje</a></td>";
			 	}
             ?>
	</tr>
</table>
