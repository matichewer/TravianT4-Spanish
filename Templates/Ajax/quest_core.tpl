...
if (isset($qact)){
 switch($qact) {
	...
	case '6':
	$submittedRank = isset($_POST['val']) ? $_POST['val'] : null;
	$currentRank = (int)$ranking->getUserRank($session->userinfo['username']);
	$rankValidated = !empty($_SESSION['quest_validated'][5]) || (is_numeric($submittedRank) && (int)$submittedRank === $currentRank);
	...
	if(!$coordinatesValidated && isset($_POST['x'], $_POST['y']) && is_numeric($_POST['x']) && is_numeric($_POST['y'])) {
		$nearestVillage = $database->getFieldDistance($database->getVillageID($session->uid));
		$nearestCoordinates = $database->getCoor($nearestVillage);
		$coordinatesValidated = (int)$_POST['x'] === (int)$nearestCoordinates['x'] && (int)$_POST['y'] === (int)$nearestCoordinates['y'];
	}
	...
	case '15':
	$submittedLumber = isset($_POST['val']) ? $_POST['val'] : null;
	$lumberValidated = !empty($_SESSION['quest_validated'][14]) || (is_numeric($submittedLumber) && (int)$submittedLumber === 210);
	...
	$submittedRank = isset($_POST['val']) ? $_POST['val'] : null;
	$currentRank = (int)$ranking->getUserRank($session->userinfo['username']);
	$rankValidated = !empty($_SESSION['quest_validated'][16]) || (is_numeric($submittedRank) && (int)$submittedRank === $currentRank);
...
<?php $_SESSION['qstnew']='0'; }else{ $_SESSION['qstnew']='1'; ?>
{"markup":"\n\t\t<div id=\"popup3\"><div id=\"qstd\"><h4>Tarea 9: ¡Vecinos!<\/h4><div class=\"spoken\">&rdquo;¡Exacto, ahí está la aldea <b> <?php echo $nvillagename;?> <\/b>! Tienes tantos recursos como los que reúne esa aldea. Bueno, casi tantos...&rdquo;<\/div><div class=\"rew\"><p class=\"ta_aw\"><input type=\"hidden\" id=\"qst_val_x\" value=\"<?php echo (int)$ncoor['x']; ?>\" \/><input type=\"hidden\" id=\"qst_val_y\" value=\"<?php echo (int)$ncoor['y']; ?>\" \/>Recompensa:<\/p><span class=\"resources r1\"><img class=\"r1\" title=\"Madera\" src=\"img\/x.gif\" alt=\"Madera\">60<\/span><span class=\"resources r2\"><img class=\"r2\" title=\"Barro\" src=\"img\/x.gif\" alt=\"Barro\">30<\/span><span class=\"resources r3\"><img class=\"r3\" title=\"Hierro\" src=\"img\/x.gif\" alt=\"Hierro\">40<\/span><span class=\"resources r4\"><img class=\"r4\" title=\"Cereal\" src=\"img/x.gif\" alt=\"Cereal\">90<\/span><\/div><br><span id=\"qst_accpt\"><a class=\"arrow\" href=\"javascript: qst_next(1,'10');\">Continuar con la siguiente tarea.<\/a><\/span><\/div>\n\t\t<div id=\"qstbg\" class=\"neighbour\"><\/div>\n\t\t","number":9,"reward":{"wood":60,"clay":30,"iron":40,"crop":90},"qgsrc":"q_l<?php echo $session->userinfo['tribe'];?>g","msrc":"<?php echo $messagelol; ?>","altstep":0}
<?php }?>
