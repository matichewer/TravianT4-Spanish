...
if($StartNatars){
	...
	for($i=1;$i<=14;$i++){
        ...
        if($status == 0) {
        	...
        	$database->addTech($wid);
        	$database->addABTech($wid);
			mysql_query("UPDATE " . TB_PREFIX . "vdata SET name = '" . mysql_real_escape_string(natarWonderVillageName($wid, $x, $y)) . "' WHERE wref = '$wid'");
			...
			// Los edificios salen de GameEngine/NatarVillage.php, no copiados aca: esta linea
			// estaba duplicada en los dos modulos del panel de administracion y al quitarle la
			// residencia se desincronizaron.
