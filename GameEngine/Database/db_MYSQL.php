<?php
		...
		function removeActiveSession($username, $sessid) {
        	...
        	function setFieldTaken($id) {
				...
				function acquireTrainingLock($vid, $timeout = 5) {
						...
						$lockName = mysql_real_escape_string(TB_PREFIX."training_".$vid,$this->connection);
						$result = mysql_query("SELECT GET_LOCK('$lockName',$timeout)",$this->connection);
						$row = $result ? mysql_fetch_row($result) : false;
						return $row && (int)$row[0] === 1;
				}
				...
				function acquireTrainingCompletionLock($timeout = 0) {
						$timeout = max(0,min(10,(int)$timeout));
						$lockName = mysql_real_escape_string(TB_PREFIX."training_completion",$this->connection);
						$result = mysql_query("SELECT GET_LOCK('$lockName',$timeout)",$this->connection);
						$row = $result ? mysql_fetch_row($result) : false;
						return $row && (int)$row[0] === 1;
				}
				...
				function getConquestEligibility($from, $target, $attackerOwner, $defenderOwner) {
					...
					// tiene que habilitarlo. Ver db_MYSQLi.php.
					$slot = 0;
					$occupied = 0;
        	...
        	*****************************************/
			...
			function syncClimberPopulation($uid) {
				...
				$q = "UPDATE " . TB_PREFIX . "users u
					...
					WHERE u.id = $uid";
				$updated = mysql_query($q, $this->connection);
			...
			function syncAllClimberPopulations() {
				...
				$allies = mysql_query("SELECT id FROM " . TB_PREFIX . "alidata", $this->connection);
				if($allies) {
		...
		References:
        	...
        	*****************************************/
			...
			/**
			 * Reloj exclusivo de la lealtad; ver db_MYSQLi.php.
			 */
			function ensureLoyaltyClockColumn() {
				...
				$result = mysql_query("SHOW COLUMNS FROM `$table` LIKE 'loyaltyupdate'", $this->connection);
				...
				if($added) {
					mysql_query("UPDATE `$table` SET loyaltyupdate = " . time() . " WHERE loyaltyupdate = 0", $this->connection);
				}
			...
			/**
			 * Cupos de expansion que habilita la residencia/palacio; ver db_MYSQLi.php.
			 */
	...
	function removeInvitation($id) {
		...
		return mysql_query($q, $this->connection);
			...
			function claimQuestResources($userid, $vref, $currentQuest, $nextQuest, $wood, $clay, $iron, $crop) {
				...
				$result = mysql_query($q, $this->connection);
				return $result && mysql_affected_rows($this->connection) === 2;
			}
			...
			function claimFollowupQuestGold($userid, $currentFquest, $nextFquest, $gold) {
				...
				$currentFquest = mysql_real_escape_string($currentFquest, $this->connection);
				$nextFquest = mysql_real_escape_string($nextFquest, $this->connection);
			...
			function markFollowupQuestAchieved($userid, $questIndex) {
				...
				for($attempt = 0; $attempt < 3; $attempt++) {
					$result = mysql_query("SELECT fquest FROM " . TB_PREFIX . "users WHERE id = $userid LIMIT 1", $this->connection);
					$row = $result ? mysql_fetch_assoc($result) : false;
					...
					$currentEscaped = mysql_real_escape_string($currentFquest);
					$nextEscaped = mysql_real_escape_string($nextFquest);
					$result = mysql_query(
		...
		function noticeUnviewed($id, $uid = null) {
			...
			function getNotice2($id, $field) {
				...
				$result = mysql_query($q, $this->connection);
				$dbarray = $result ? mysql_fetch_assoc($result) : false;
