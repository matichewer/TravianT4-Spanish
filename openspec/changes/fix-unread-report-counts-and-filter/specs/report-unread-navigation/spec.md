...
The system SHALL display route badges in a clearly visible muted light gray, trade badges in dark gray, and adventure badges in green; no other report category SHALL use the adventure green.
...
- **THEN** the navigation distributes the tabs according to their label width across the complete bar without clipping labels, overflowing the fixed-width report panel, or leaving uncovered side gaps
